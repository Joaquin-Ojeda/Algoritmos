...
{
   int PROGRESS_TYPE=1;
   return {PROGRESS_TYPE,"",0,0,1};
}

Progress progress(int barSize,char fillChar)
{
   int PROGRESS_TYPE=2;
   return {PROGRESS_TYPE,"",barSize,fillChar,1};
}

void progressStart(Progress& p,string mssg)
{
   p.mssg=mssg;
...
void progressDemo()
{
   ...
   progressStart(p1,"Demo progress tipo 1:");
   for(int i=0; i<100; i++)
   {
      progressShow(p1,i);
   ...
   // PROGRESS TIPO 2
   Progress p2 = progress(30,'#');
   progressStart(p2,"Demo progress tipo 2:");
   for(int i=0; i<100; i++)
   {
      progressShow(p2,i);
